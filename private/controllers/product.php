...
if(empty($admin)){
    ...
    if (Helper::is_post()) {
        ...
        if(empty($_POST['id'])){
            ...
            $product->ar_image = trim($_POST['ar_prev_image']);
            $product->dim_image = trim($_POST['dim_prev_image']);
            $product->ar_featured = (isset($_POST['ar_featured'])) ? 1 : 2;
            $product->dim_featured = (isset($_POST['dim_featured'])) ? 1 : 2;
            ...
            if($errors->is_empty()){
                ...
                if(!empty($_FILES["ar_img"]["name"])){
                    $upload = new Upload($_FILES["ar_img"]);
                    $upload->set_max_size(MAX_IMAGE_SIZE);
                    if($upload->upload()){
                        $upload->delete($product->ar_image);
                        $product->ar_image= $upload->get_file_name();
                        $product->ar_image_resolution = $upload->resolution;
                    }
                    $errors = $upload->get_errors();
                }

                if(!empty($_FILES["dim_img"]["name"])){
                    $upload = new Upload($_FILES["dim_img"]);
                    $upload->set_max_size(MAX_IMAGE_SIZE);
                    if($upload->upload()){
                        $upload->delete($product->dim_image);
                        $product->dim_image= $upload->get_file_name();
                        $product->dim_image_resolution = $upload->resolution;
                    }
                    $errors = $upload->get_errors();
                }
                if($errors->is_empty()){
...
if(empty($admin)){
    ...
    if (Helper::is_post()) {
        ...
        if(empty($_POST['id'])){
            ...
            // $product->validate_except(["image_name", "image_resolution", "sell", "inventory","purchase_price", "prev_price"]);
